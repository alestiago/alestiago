...
﻿### Diagrams (Mermaid – great for state machines)

﻿You can describe diagrams (including state machines) using Mermaid-style fenced code blocks, which are then rendered client-side:
...
﻿Any fenced block with the `mermaid` language will be turned into a `<div class="mermaid">...</div>` and rendered by Mermaid.js in the generated HTML.
